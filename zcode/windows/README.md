...
Enable the hidden CDP (Chrome DevTools Protocol) debugging channel for the built-in browser of **ZCode Desktop 3.10.1 / 3.10.2 / 3.12.3 / 3.14.0** (Windows x64). After patching, the AI can use `tab.cdp.*` and `tab.openDevTools()` on in-app browser tabs: arbitrary CDP commands, event streams, breakpoints / pause / resume, and opening DevTools.
...
Double-click the repo-root `DevToolsTool.exe` for a unified interactive menu (① install ② backup ③ remove ④ status, ⑤ switch ZCode/WorkBuddy target, ⑥ language 中文/English) with a progress bar — Chinese UI by default.
...
- Targets **3.10.1 / 3.10.2 / 3.12.3 / 3.14.0** via content anchors. After a ZCode auto-update, run `Status` first.
